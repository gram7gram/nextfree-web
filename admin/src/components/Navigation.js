    ...
    return <nav className="navbar navbar-expand-lg navbar-dark bg-primary">
      ...
      <div className={"navbar-collapse collapse" + (this.state.isNavOpen ? " show" : "")}>
        <ul className="navbar-nav mr-auto text-center">
          ...
          {isAuthenticated && <li className="nav-item">
            <Link to={Pages.OWNERS} className="nav-link text-white">{i18n.t('navigation.owners')}</Link>
          </li>}

          {isAuthenticated && <li className="nav-item">
            <Link to={Pages.CUSTOMERS} className="nav-link text-white">{i18n.t('navigation.customers')}</Link>
          </li>}
