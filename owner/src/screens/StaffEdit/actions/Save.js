...
export default (model) => (dispatch, getState) => {
  ...
  if (model.id) {
    promise = request.put(parameters.apiHost + `/api/v1/owner/companies/${company._id}/staff/${model.id}`, data, {
      headers: {
        Authorization: token
      }
    })
  } else {
    promise = request.post(parameters.apiHost + `/api/v1/owner/companies/${company._id}/staff`, data, {
      headers: {
        Authorization: token
      }
    })
